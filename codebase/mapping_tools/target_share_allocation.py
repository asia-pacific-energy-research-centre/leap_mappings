...
def apply_target_dataset_allocation(
    ...
    rows["_source_year"] = pd.to_numeric(rows["year_key"], errors="coerce")

    # ESTO is historical. For a projected Ninth year, an exact same-year
    # target basis is often absent (or contains only zeroes), so an equal split
    # would discard the observed economy-specific product composition. In that
    # case use the latest available target year not later than the source year,
    # shared by every candidate target in the source group. Same-year positive
    # target values always retain precedence.
    historical_basis = basis.rename(
        columns={"year_key": "_basis_year_key", "target_basis": "_historical_basis"}
    ).copy()
    historical_basis["_basis_year"] = pd.to_numeric(
        historical_basis["_basis_year_key"], errors="coerce"
    )
    fallback_candidates = rows[
        ...
        on=["economy_key", "target_flow", "target_product"],
        how="left",
    )
    fallback_candidates = fallback_candidates.loc[
        fallback_candidates["_basis_year"].le(fallback_candidates["_source_year"])
    ].copy()
    latest_profile_year = (
        fallback_candidates.groupby("_source_group_key", dropna=False)["_basis_year"]
        .max()
        .rename("_profile_year")
        .reset_index()
    )
    fallback_basis = fallback_candidates.merge(
        latest_profile_year,
        on="_source_group_key",
        how="inner",
    )
    fallback_basis = fallback_basis.loc[
        fallback_basis["_basis_year"].eq(fallback_basis["_profile_year"])
    ]
    fallback_basis = fallback_basis[
        ["_source_group_key", "target_flow", "target_product", "_historical_basis"]
    ].drop_duplicates(
        subset=["_source_group_key", "target_flow", "target_product"]
    )
    rows = rows.merge(
        fallback_basis,
        on=["_source_group_key", "target_flow", "target_product"],
        how="left",
    )

    current_pair_basis = rows[
        ["_source_group_key", "target_flow", "target_product", "target_basis"]
    ].drop_duplicates(
        subset=["_source_group_key", "target_flow", "target_product"]
    )
    current_totals = current_pair_basis.groupby(
        "_source_group_key", dropna=False
    )["target_basis"].sum()
    use_historical_basis = rows["_source_group_key"].map(current_totals).fillna(0.0).le(0)
    rows.loc[use_historical_basis, "target_basis"] = rows.loc[
        use_historical_basis, "_historical_basis"
    ].fillna(0.0)
