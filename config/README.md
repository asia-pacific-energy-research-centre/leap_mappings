...
1. `outlook_mappings_single_axis.xlsx` is the editable mapping authority. Maintain
   ...
   at least one accepted exact pair in the pair sheet for each applicable
   dataset scope. Use the narrow `extra_*_key_pairs` sheets for valid structural
   pairs that current data does not establish; never add a full flow-by-product
   cross product. Follow `docs/esto_extended_category_creation_considerations.md`
   for the complete workflow and worked PHEV example.
