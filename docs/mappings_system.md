...
Maps LEAP/ESTO/9th data into comparison rows and aggregates values. It takes ESTO-shaped source data, maps every row to a `common_row_id`, aggregates values within each common row, and flags common rows that are unusually broad or aggregate across many components.

Output: `results/common_esto/common_esto_comparison_data.csv`

If Stage 3 detects mapping-review blockers such as broad common rows or intersecting
axis groups, the latest outputs are written with a `_needs_mapping_review` suffix.
Check `results/common_esto/common_esto_output_status.csv` to see which files belong
to the latest run.
...
Broad common rows generated by graph partitioning are not automatically wrong. They may be the correct result when a source aggregate overlaps several detailed ESTO rows. Review them to confirm they come from real source aggregate constraints rather than missing mappings.
