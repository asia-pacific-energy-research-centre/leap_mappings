...
### Separate-axis generation workbooks
...
sector/flow and fuel/product relations, accepted exact-pair authority, and
generated compatibility views. Extra comparison logic still belongs in rollup
...
For ESTO target rollups, the preferred path is to register the rolled flow as a real hierarchy node
by filling `parent_flow_label` and `child_flow_labels` in `esto_rollup_rules`. Registered rolled
targets stay as single full-value relationship rows and are added to both the ESTO and Common ESTO
flow trees for Stage 3 parent/child validation. Fallback expansion with `allocation_share` is only
...
| `parent_flow_label` / `child_flow_labels` | For ESTO target rollups, declares the rolled flow's hierarchy position. Populating both makes the rolled label a registered tree node rather than a fallback expanded target. |
...
Explicit rollups and cross-dataset common comparison categories have different naming rules.

**Explicit rollups** (in `leap_rollup_rules`, `esto_rollup_rules`, `ninth_rollup_rules`) are human-defined rules for known categories. Naming depends on the axis being rolled:

- **ESTO target rollups**: follow ESTO code format. Use the compressed code-range convention (e.g. `16.01-16.02 Buildings`) and prefer a real parent category code where one exists.
...
pair-sheet contracts. The old pair master is used only for bootstrap,
comparison, shared subtotal metadata, rollup rules, and unchanged non-pair
sheets.
...
There is no active general Stage 0. Run the focused hierarchy/subtotal or
missing-ESTO-row review workflow only when its underlying structural evidence
changed. These workflows publish review artifacts and do not silently mutate
