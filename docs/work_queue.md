...
- **Prototype checkpoint:** The prototype is now split into three config workbooks: a 585-row, six-sheet user-maintained axis workbook; a generated do-not-edit pair registry with 48,270 auditable combinations; and a generated do-not-edit compatibility master. The LEAP registry is no longer circular: it contains 1,993 structural pairs from all 21 current economy templates plus the detailed demand/power rows, with a 22-workbook fingerprint manifest and automatic cached refresh. The compatibility master preserves all 14 canonical sheet names and their order, leaves the 11 non-pair sheets unchanged in cell content, exposes only the canonical pair-sheet columns, and passes the current loader and Stage 1 without a code change (2,426 active relationships; 4,852 Stage 1 rows). The axes still contain eight blocking within-axis many-to-many components. The structural census also proves that literal model branches are not the whole LEAP result-key contract: only 635 of 2,834 current source pairs are literal registry members; 2,199 balance-interface or rollup keys require a deterministic second layer rather than a mapping-sheet bootstrap.
- **Next action:** Add the deterministic LEAP balance-key layer from the existing balance parser and hierarchy/rollup rules, using observed balance exports as verification rather than global authority. Then review the eight blocking axis components and temporal disagreement families, decide whether conditional fuel meanings require context-qualified relations, and make the compiler read the editable axis workbook directly.
