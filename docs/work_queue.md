...
ESTO Extended category, generated `data/esto_extended_catalogue.csv` must contain it;
...
`ESTO_EXTENDED` label; `esto_extended_catalogue.csv` is treated only as a structurally
...
**Checkpoint 2 — mappings foundation complete (2026-09-03):** Added the
numeric-free `data/esto_extended_catalogue.csv` contract and a deterministic
coverage validator over active Extended mapping and rollup pairs. Production
value-adapter registration no longer runs an Extended exact-row extractor;
Stage 3 aliases ordinary ESTO facts into Extended scopes and requires the
catalogue before publication. The prior `data/esto_extended.csv`, per-vintage
tables, registry, and exact-row artifact remain untouched legacy compatibility
files: they are explicitly deprecated and must not be regenerated by the new
builder. Power certification now distinguishes structural target/component
definitions from ordinary-ESTO component coverage instead of comparing against
an Extended numeric series. Numeric source rows carry
...
replace or delete the legacy numeric files until those consumers have migrated
and their compatibility tests pass.
