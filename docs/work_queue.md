...
- **Next action:** Make the compiler read the editable single-axis workbook directly instead of deriving axes from the maintained pair sheets. Then run the generated 11,150-relationship master through the existing Stage 1-3 shadow pipeline and record schema, graph-membership, subtotal, and source-once differences. Review the eight blocking axis components and individual exclusions after that integration path is working.
