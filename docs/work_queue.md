...
- **Priority / status / timing:** P2 · `shadow_compiler_implemented_promotion_blocked` · merge exploration, defer canonical promotion
...
- **Safety:** Keep the generated pair and compatibility workbooks review-only. The structural Stage 3 gate finds 3,007 generated source-pair/scope groups reaching multiple Common ESTO rows, versus 177 canonical; 2,830 failures are new. A full value run reached about 9 GB RAM during Ninth conversion and was stopped to protect the workstation. More RAM would not resolve the source-once semantics. Do not replace the canonical workbook or migrate `leap_initialisation` / `leap_dashboard` consumers.
...
- **Next action:** Treat the compiler/workbooks/docs as a mergeable isolated further-development feature, not a production switch. Before promotion, resolve the 2,830 new source-once fan-outs by narrowing axes, reviewing provisional relationships, or adding explicit allocation where a real split is intended. Then rerun Stage 1-3. The eight within-axis many-to-many components and the large Stage 2 graph/performance increase remain explicit review debt.
