...
- **Prototype checkpoint:** The prototype is now split into three config workbooks: a 585-row, six-sheet user-maintained axis workbook; a generated do-not-edit pair registry with 83,451 auditable combinations; and a generated do-not-edit compatibility master. Raw pair discovery is followed by deterministic expansion of the active canonical rollup rules for LEAP, ESTO, ESTO Extended, and Ninth. The generated workbook exposes only `pair_origin` (`raw`, `rollup`, or `raw_and_rollup`) for provenance. This adds 700 LEAP, 1,258 ESTO, 861 ESTO Extended, and 554 Ninth exact rollup pairs. The compatibility master preserves all 14 canonical sheet names and their order, leaves the 11 non-pair sheets unchanged in cell content, exposes only the canonical pair-sheet columns, and passes the current loader and Stage 1 without a code change (6,290 active relationships; 12,580 Stage 1 rows). The rollup layer restores 333 of the 758 current LEAP source pairs previously missing from raw authority and 206 Ninth source pairs. The remaining source-authority queue is 425 unique LEAP pairs and 40 unique Ninth pairs; overall missing maintained relationships fall from 4,373 to 3,181. The axes still contain eight blocking within-axis many-to-many components.
- **Next action:** Review the eight blocking axis components and the refreshed temporal-gap/subtotal workbook together. Start with internally mixed subtotal flags, then classify the remaining 425 LEAP and 40 Ninth source-authority pairs, especially older path aliases and component labels, before changing temporal policy. Decide whether conditional fuel meanings require context-qualified relations and make the compiler read the editable axis workbook directly.
