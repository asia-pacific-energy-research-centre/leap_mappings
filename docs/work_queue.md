...
- **Prototype checkpoint:** The prototype is now split into three config workbooks: a 585-row, six-sheet user-maintained axis workbook; a generated do-not-edit pair registry with 49,111 auditable combinations; and a generated do-not-edit compatibility master. The compatibility master preserves all 14 canonical sheet names and their order, leaves the 11 non-pair sheets unchanged in cell content, exposes only the canonical pair-sheet columns, and passes the current loader and Stage 1 without a code change (5,141 active relationships; 10,282 Stage 1 rows). The current bootstrap still contains eight blocking within-axis many-to-many components. The all-exact-pair compiler matches 6,239 current relationships but adds 1,923 extra targets on current source pairs; the boundary/future compiler matches 4,071 and adds 963. LEAP pair authority remains a labelled bootstrap until the model-branch parser is supplied.
- **Next action:** Review the eight blocking axis components and the temporal disagreement families. Decide whether conditional fuel meanings can be resolved entirely by accepted exact-pair membership or require a small context-qualified axis relation. Then make the compiler read the editable axis workbook directly and replace the LEAP bootstrap with the instructed branch parser when its contract is provided.
