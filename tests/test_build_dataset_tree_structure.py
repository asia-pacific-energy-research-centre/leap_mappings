...
def test_common_validation_processes_source_systems_sequentially(tmp_path: Path) -> None:
    comparison_path = tmp_path / "comparison.csv"
    tree = pd.DataFrame([
    ...
    rows = []
    for source_system in ["ESTO", "NINTH"]:
        for product, value in [("P", 11.0), ("P.1", 4.0), ("P.2", 6.0)]:
            rows.append({
                "comparison_scope": "scope", "source_system": source_system,
                "economy": "20_USA", "scenario": "reference", "year": 2023,
                "common_flow_label": "F", "common_product_label": product,
                "value": value,
            })
    pd.DataFrame(rows).to_csv(comparison_path, index=False)
    ...
    assert result["source_system"].tolist() == ["ESTO", "NINTH"]
