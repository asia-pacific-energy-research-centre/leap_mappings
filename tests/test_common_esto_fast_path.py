...
def test_fast_path_can_filter_to_one_economy(tmp_path: Path) -> None:
    relationship_dir = tmp_path / "results" / "mapping_relationships"
    common_dir = tmp_path / "results" / "common_esto" / "economy_scoped" / "02BD"
    source_paths = {
        "LEAP": relationship_dir / "leap_results_converted_to_esto.csv",
        "NINTH": relationship_dir / "ninth_results_converted_to_esto.csv",
        "ESTO": relationship_dir / "esto_results_exact_rows.csv",
    ...
    for source_system, path in source_paths.items():
        source_economy = "02BD" if source_system == "ESTO" else "02_BD"
        _write_csv(
            path,
            [
                {
                    ...
                    "scenario": "Target" if source_system == "LEAP" else "reference",
                    "year": 2024,
                    "esto_flow": "F1",
                    "esto_product": "P1",
                    "value": 2,
    ...
    common_rows_path = tmp_path / "results" / "common_esto" / "common_esto_rows.csv"
    _write_csv(
        common_rows_path,
        [
            {
    ...
    comparison_df, wide_df, _ = run_common_esto_comparison_fast_path(
        source_paths=source_paths,
        common_rows_path=common_rows_path,
        output_dir=common_dir,
        default_economy="20_USA",
