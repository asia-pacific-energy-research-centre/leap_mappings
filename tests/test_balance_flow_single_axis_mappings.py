...
def test_single_axis_ninth_other_hydrocarbons_does_not_broaden_its_targets() -> None:
    ...
    assert products == {"06.03 Refinery feedstocks", "06.04 Additives/ oxygenates"}
...
def test_leap_smr_with_ccs_temporarily_maps_to_both_smr_esto_children() -> None:
    ...
    assert (
        ...
        "15.02.02.02.03 BEV small LPV",
        "ESTO_EXTENDED",
    ) in flow_rows
    smr_rows = {
        row
        for row in flow_rows
        if row[0].replace("/Processes/", "/")
