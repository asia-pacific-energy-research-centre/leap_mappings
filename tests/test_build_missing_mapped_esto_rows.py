...
def test_always_required_lng_rows_cover_economies_and_nonzero_0906_fuels(tmp_path: Path) -> None:
    ...
    # Natural gas and LNG are always required; Fuel oil is discovered from non-zero Ninth 09.06 data.
    expected_products = {
        "01 Coal",
        "01.01 Coking coal",
        "07.08 Fuel oil",
    ...
    # Split flows are leaves, so subtotal status comes from the product hierarchy.
    liquefaction = rows[rows["flows"].eq("09.06.02.01 Liquefaction")]
    assert set(liquefaction.loc[liquefaction["products"].eq("01 Coal"), "is_subtotal"]) == {"TRUE"}
    assert set(liquefaction.loc[liquefaction["products"].eq("01.01 Coking coal"), "is_subtotal"]) == {"FALSE"}
...
def test_structural_completion_covers_every_parent_economy_product_without_ninth_evidence(tmp_path: Path) -> None:
    ...
    assert completion["source_ninth_sector"].eq("").all()


def test_output_is_source_shaped_zeroed_and_non_subtotals_are_uppercase_false(tmp_path: Path) -> None:
    workbook_path, ninth_path, esto_path = _fixture_paths(tmp_path)
    ...
    leaf_rows = rows[
        ...
        | (
            ...
            & rows["products"].isin(["01.01 Coking coal", "07.08 Fuel oil", "08.01 Natural gas", "08.02 LNG"])
        )
    ]
